		...
		<div class="container">
			<div class="row">
				<div class="col-lg-8 mx-auto">
					...
					<h2>Decryption Operation</h2>
					<hr/>
					
					<h3>Decryption of Plain Text</h3>
					<hr/>
					
					<%
						String decText = request.getParameter("decText");
						if (decText != null)
						{
							%>
					...
					<form action='decrypt' >
						...
						<label>Enter the text to be decrypted</label>
						<input type=text name='text' class='form-control' placeholder="Encrypted Text" />
						<br/>
					...
					<form action='decrypt?req_type=textfile' method="post" enctype="multipart/form-data">
						<label>Chose the file to be decrypted</label>
					...
					<%
						...
						try 
						{
							List<DecryptionHistory> history = eDao.read(uModel.getEmail());
							if (history != null && history.size() > 0)
							{
								%>
									<table class='table'>
										...
										<% 
											for (DecryptionHistory eh: history)
											{
												%>
														<tr>
															<td> <%=eh.getEmail() %></td>
						...
						catch (Exception e)
						{
							%>
								...
								<h4>Exception while trying to fetch the Decryption History</h4>
